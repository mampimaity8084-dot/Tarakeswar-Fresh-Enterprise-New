...
The old browser-callable create_order_v3 is revoked for anon users. The new Netlify Functions use a Supabase SECRET key. Never put that key in index.html or supabase-config.js.

ONE-TIME SETUP (ONLY 3 THINGS)
1) Supabase SQL Editor: run UPGRADE_V3_SECURE_PREMIUM.sql once.
2) Netlify > Environment variables: add ONE new secret variable:
...
3) Upload this whole ZIP to Netlify Drop as a new production deploy.

DO NOT SHARE OR SCREENSHOT SUPABASE_SECRET_KEY / service_role / RAZORPAY_KEY_SECRET.

PAYMENT FLOW
Customer cart -> Netlify create-checkout -> server reads real product prices from Supabase -> Razorpay order -> payment -> Netlify verifies HMAC + Razorpay payment amount/status -> captures if required -> private Supabase RPC creates paid order -> customer receives Order ID + Delivery OTP.
