/* Tarakeswar Fresh Fish V6 FINAL — Fixed UX, AI Studio, Realtime & Navigation */
(function(){'use strict';
const $=id=>document.getElementById(id);

function isCustomer(){
  return !location.pathname.toLowerCase().includes('admin') && !location.pathname.toLowerCase().includes('delivery');
}
function isAdmin(){
...
function style(){
  if($('tffV6Style'))return;
  const s=document.createElement('style');
  s.id='tffV6Style';
  s.textContent=`
/* V6 cleanup: canonical bottom bar navigation */
...
.tff-v6-customer .appNav{bottom:0!important;display:flex!important}
...
function addProductStudio(){
  if(!isAdmin()||$('tffV6ProductStudio'))return;
...
function enhanceCustomerAI(){
  ...
  document.body.classList.add('tff-v6-customer');
  const input=$('tffAiInput');
...
function realtime(){if(!window.sb?.channel)return;try{const channel=window.sb.channel('tff-v6-realtime');channel.on('postgres_changes',{event:'*',schema:'public',table:'products'},()=>{if(isCustomer()&&typeof window.loadProducts==='function')window.loadProducts()}).on('postgres_changes',{event:'*',schema:'public',table:'fish'},()=>{if(isCustomer()&&typeof window.loadProducts==='function')window.loadProducts()}).on('postgres_changes',{event:'*',schema:'public',table:'orders'},()=>{if(isCustomer()&&typeof window.trackOrder==='function'){const r=$('trackOrderNo');if(r&&r.value)window.trackOrder()}}).on('postgres_changes',{event:'*',schema:'public',table:'offers'},()=>{if(isCustomer()&&typeof window.loadOffers==='function')window.loadOffers()}).subscribe()}catch(e){console.warn('realtime',e)}}
function smartStock(){if(!isCustomer()||!window.tffProducts)return;document.querySelectorAll('.card').forEach(card=>{const text=card.textContent||'';if(/Only 0 left/i.test(text))card.classList.add('tff-v6-out')})}
